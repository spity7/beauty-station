...
| `/shop`                       | Published products from API; URL-driven filters (`categoryId`, `brandId`, `search`, `sort`, `minPrice`, `maxPrice`, `page`, `limit`); sidebar/search/sort/page size navigate via query params; empty state when no matches; error message when API unreachable (no demo catalog fallback) |
